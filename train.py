...
def main():
    ...
    model = FlowEstimator(opt.train['planes'])
    model = nn.DataParallel(model).cuda()
    
    optimizer = AdamW(model.parameters(), lr=opt.train['lr'], weight_decay=opt.train['weight_dceay'])

    train_set = Vimeo90K(opt.data_root, 'train')
    val_set = Vimeo90K(opt.data_root, 'val')
    ...
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir, exists_ok=True)
    ...
    for e in range(1, opt.train['epochs']+1):
        ...
        for (img0, img1, gt) in tqdm(train_loader):
            ...
            if global_step % opt.train['print_every'] == 0
                tqdm.write(f'[Epoch {e} Iter {global_step}/{total_steps}] Rec: {loss_rec.item():.4f}\t Cons: {loss_cons.item():.4f}\t LR: {lr}')
            ...
            if global_step % opt.train['viz_every'] == 0:
                for i in range(4):
                    img0 = (img0_0[i].permute(1,2,0).detach().cpu().numpy() * 255).astype(np.uint8)
                    img1 = (img0_1[i].permute(1,2,0).detach().cpu().numpy() * 255).astype(np.uint8)
                    gt = (gt_0[i].permute(1,2,0).detach().cpu().numpy() * 255).astype(np.uint8)
                    pred = (frame_0[i].permute(1,2,0).detach().cpu().numpy() * 255).astype(np.uint8)
                    writer.add_image(f'vis_{i}/img0', img0, global_step)
                    writer.add_image(f'vis_{i}/img1', img1, global_step)
                    writer.add_image(f'vis_{i}/gt', gt, global_step)
                    writer.add_image(f'vis_{i}/pred', pred, global_step)
